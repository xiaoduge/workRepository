...
void Configure_SN74ALVC164245(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA |
                         ...
                         RCC_APB2Periph_GPIOD, ENABLE);  //使能APB2外设时钟，ENABLE改为DISABLE为失能APB2外设时钟
  /*
  1A1 -> PA12(Reset)
  1A2 -> PB0(E1)
  1A3 -> PB1(E2)
  ...
  */
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; //普通推挽输出，一般用在0V和3.3V的场合
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; //上拉输入 ，GPIO_Mode_IPD为下拉输入
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(GPIOC, &GPIO_InitStructure);
  
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_2 | GPIO_Pin_3 |
                                GPIO_Pin_4 | GPIO_Pin_5 | GPIO_Pin_6 | GPIO_Pin_7;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD;  //开漏输出，一般用在电平不匹配的场合，比如需要输出5V的电平，外部接上拉电阻，电源为5V
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;
  GPIO_Init(GPIOC, &GPIO_InitStructure);
